# Skill: roadmap-planning

Turn product context into a durable roadmap, release plan, sequencing recommendation, or now/next/later
view.

## Templates and references

Before using the default structure, check:

- [templates/roadmap.md](templates/roadmap.md) for the current roadmap template.
- [references/](references/INDEX.md) for PM-provided roadmap, release plan, sequencing, or planning
  examples.

If the PM adds a better roadmap template or reference example, follow that and keep this skill's
sequencing rules.

## How to do it well

- Roadmaps should connect strategy to execution: outcomes, PRDs, decisions, design, and data.
- Make sequencing explicit. Dependencies and confidence matter as much as dates.
- Use dates only when there is a real commitment or useful planning horizon.
- Link follow-on stakeholder updates when a roadmap should be communicated.

## Filing

Write to `product-docs/roadmaps/<MMM-DD-YYYY>-<topic>.md`, update
[the roadmaps index](../product-docs/roadmaps/INDEX.md), and apply
[apply-pmos-struct](apply-pmos-struct.md).
